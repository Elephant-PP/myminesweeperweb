...
const LANGUAGES = {
  zh: {
    title: '扫雷',
    beginner: '初级', intermediate: '中级', expert: '专家',
    footer: '左键挖开 · 右键标旗 · <span class="key-hint">R</span> 重置',
    won: '恭喜通关！', lost: '踩雷了！',
    playAgain: '再来一局',
    timeUsed: '用时 {s} 秒',
    mineCount: '{label} · {r}×{c} · {n}雷',
  },
  en: {
    title: 'Minesweeper',
    beginner: 'Beginner', intermediate: 'Intermediate', expert: 'Expert',
    footer: 'Left-click to reveal · Right-click to flag · <span class="key-hint">R</span> reset',
    won: 'You Win!', lost: 'Game Over!',
    playAgain: 'Play Again',
    timeUsed: 'Time: {s}s',
    mineCount: '{label} · {r}×{c} · {n} mines',
  },
};

let currentLang = localStorage.getItem('ms-lang') || 'zh';

function t(key, params = {}) {
  let str = LANGUAGES[currentLang][key] ?? LANGUAGES.zh[key] ?? key;
...
function applyLanguage() {
  ...
  // Update footer (innerHTML — contains HTML)
  document.getElementById('game-footer').innerHTML = t('footer');
  // Update modal button
  document.getElementById('modal-btn').textContent = t('playAgain');
  // Update lang switcher buttons
  document.querySelectorAll('.lang-btn').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.lang === currentLang);
  });
  // Update difficulty info labels (remain unchanged — always numbers)
}
...
function endGame(won, newlyRevealed = []) {
  ...
  setTimeout(() => {
    ...
    modalTitle.textContent = won ? t('won') : t('lost');
    modalTitle.style.color = won ? '#64c8ff' : '#ff4444';
    const cfg = DIFFICULTIES[difficulty];
    modalStats.textContent = t('mineCount', {
      label: t(difficulty), r: cfg.rows, c: cfg.cols, n: cfg.mines,
    });
    modalDetail.textContent = t('timeUsed', { s: seconds });
